//! SearchInput — real single-line text input for the library filter.
//! Implements the platform input contract (EntityInputHandler) so it supports
//! caret movement, selection, clipboard, and IME composition.
...
impl SearchInput {
    pub fn new(cx: &mut Context<Self>) -> Self {
        Self::with_placeholder(cx, "Type to filter tracks…")
    }

    /// Generic single-line input with a custom placeholder — reused by the
    /// playlist name field and the tag editor.
    pub fn with_placeholder(cx: &mut Context<Self>, placeholder: impl Into<SharedString>) -> Self {
        Self {
    ...
    /// Set initial content (e.g. current tag value in the tag editor).
    /// Does not emit `Changed` — callers decide whether that means anything.
    ...
    pub fn clear(&mut self, cx: &mut Context<Self>) {
        if self.content.is_empty() && self.selected_range.is_empty() {
            return;
        }
        self.content = "".into();
    ...
    fn submit(&mut self, _: &SearchSubmit, _: &mut Window, cx: &mut Context<Self>) {
        cx.emit(SearchEvent::Submit);
    }
    ...
    fn tab_next(&mut self, _: &InputTabNext, _: &mut Window, cx: &mut Context<Self>) {
        cx.emit(SearchEvent::TabNext);
    }

    fn tab_prev(&mut self, _: &InputTabPrev, _: &mut Window, cx: &mut Context<Self>) {
        cx.emit(SearchEvent::TabPrev);
    }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color): (SharedString, gpui::Hsla) = if content.is_empty() {
            (input.placeholder.clone(), tokens.faint.into())
        } else {
            (content, style.color)
        };
    ...
    fn paint(
        ...
        if let Err(error) = line.paint(
            ...
            eprintln!("search input paint failed: {error}");
        }
